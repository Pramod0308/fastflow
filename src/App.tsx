...
export default function App() {
  ...
  const expectedEnd = current ? new Date(current.startAt + targetHours * 3600000) : undefined;
  ...
  const stage = useMemo(() => {
    ...
    return reached[reached.length - 1]; // last reached stage (undefined if <3h)
  }, [hours]);
  ...
  const openEdit = () => { if (current) { setEditValue(dayjs(current.startAt).format('YYYY-MM-DDTHH:mm')); setOpenEditStart(true); } };
  const saveEdit = () => {
    if (!current) return;
  ...
  const start = (plan: string) => startFast(PLAN_TO_HOURS[plan] ?? settings.defaultTargetHours, plan);
          ...
          <CardContent sx={{ display: 'grid', placeItems: 'center', pt: 2 }}>
            ...
            <Box sx={{ color: 'primary.main', position: 'relative' }}>
              ...
              <Box sx={{ position: 'absolute', inset: 0, display: 'grid', placeItems: 'center' }}>
                {/* Stage icon + timer in a stack */}
                <Stack spacing={1} alignItems="center">
                  {stage && <Box sx={{ fontSize: 28, lineHeight: 1 }}>{stage.icon}</Box>}
                  <Typography
                    variant="h3"
                    sx={{ fontWeight: 900, color: 'secondary.main', textShadow: '0 2px 8px rgba(0,0,0,.12)' }}
                  >
                    {current ? elapsedText : '00:00:00'}
                  </Typography>
