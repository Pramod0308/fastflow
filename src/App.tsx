...
import { AppBar, Toolbar, Typography, Container, Box, Button, IconButton, Menu, MenuItem, Stack, Chip, Dialog, DialogTitle, DialogContent, DialogActions, TextField, FormControlLabel, Switch, Grid } from '@mui/material';
...
export default function App(){
  ...
  const elapsedMs=current? now-current.startAt : 0; const elapsedText=prettyHms(elapsedMs);
  const targetHours=current?.targetHours ?? settings.defaultTargetHours;
  const hours=msToH(elapsedMs); const expectedEnd=current? new Date(current.startAt + targetHours*3600000) : undefined;

  // Eating timer (when not fasting)
  ...
  return (<>
    ...
    <Container maxWidth="sm" sx={{ py:2 }}>
      {/* Timer / Status in a glass, collapsible card */}
            ...
            <Box sx={{ position:'relative', display:'grid', placeItems:'center' }}>
              ...
              <Box sx={{ position:'absolute', inset:0, display:'grid', placeItems:'center' }}>
                <Stack spacing={1} alignItems="center">
                  ...
                  <Typography variant="h3" sx={{ fontWeight:900, color:'secondary.main', textShadow:'0 2px 8px rgba(0,0,0,.12)' }}>{elapsedText}</Typography>
            ...
            <Box sx={{ position:'relative', display:'grid', placeItems:'center' }}>
              ...
              <Box sx={{ position:'absolute', inset:0, display:'grid', placeItems:'center' }}>
                <Stack spacing={1} alignItems="center">
                  ...
                  <Typography variant="h3" sx={{ fontWeight:900, color:'warning.main', textShadow:'0 2px 8px rgba(0,0,0,.12)' }}>{eatingElapsedText}</Typography>
      ...
      <GlassCard title="Stats" defaultExpanded right={<HistoryIcon fontSize="small"/>}>
        ...
        <Box sx={{ mt: 1 }}><HistoryBarChart data={historyData} /></Box>
      </GlassCard>
    ...
    {/* Settings / Import / Edit start dialogs (unchanged) */}
    ...
    <Dialog open={openEditStart} onClose={()=>setOpenEditStart(false)} fullWidth>
      ...
      <DialogContent dividers><TextField type="datetime-local" fullWidth value={editValue} onChange={e=>setEditValue(e.target.value)} helperText="Set when this fast actually started (local time). Future times are not allowed."/></DialogContent>
